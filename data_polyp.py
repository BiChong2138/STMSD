...
class SalObjDataset(data.Dataset):
    def __init__(self, root, trainsize, clip_len=5):
        ...
        gt_root = os.path.join(root, "Train_GT")
        ...
        for vid in vid_list:
            ...
            gt_path = os.path.join(gt_root, vid)
            if os.path.exists(vid_path) and os.path.exists(gt_path):
                ...
                for idx in range(len(frms)):
                    ...
                    self.gts.append([x.replace("Train", "Train_GT") for x in clip])
...
class SalObjTestDataset(data.Dataset):
    def __init__(self, root, trainsize, clip_len=5):
        ...
        gt_root = os.path.join(root, "Test_GT")
        vid_list = os.listdir(image_root)
            ...
            if os.path.exists(vid_path) and os.path.exists(gt_path):
                ...
                for idx in range(len(frms)):
                    ...
                    for ii in range(-clip_len//2+1, clip_len//2+1):
                        ...
                        clip.append(os.path.join(vid_path, frms[pick_idx]))
                    self.images.append(clip)
                    self.gts.append([x.replace("Test", "Test_GT") for x in clip])
...
def get_trainloader(image_root, batchsize, trainsize, clip_len=5, shuffle=True, num_workers=8, pin_memory=True):
...
def get_testloader(image_root, batchsize, trainsize, clip_len=5, shuffle=False, num_workers=8, pin_memory=True):
