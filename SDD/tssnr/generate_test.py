...
def generate_test_dataset():
    """生成按信噪比划分的测试数据集"""
    # 参数列表
    snr_levels = [0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 3.0, 5.0]  # 8个信噪比级别
    velocity_levels = [0.5, 1, 3, 5, 7]  # 五个线速度级别
    sigma_levels = [1.25, 1.50, 1.75, 2.00, 2.25]  # 五个弥撒半径级别
    
    image_size = 256  # 256×256分辨率
    frames_per_sequence = 5
    sequences_per_snr = 100  # 每个SNR级别100个序列
    
    # 为每个SNR级别生成测试数据
    for snr_idx, snr in enumerate(tqdm(snr_levels, desc="SNR Levels")):
        # 创建SNR目录
        snr_str = f"{int(snr*100):03d}"
        snr_dir = f"SNR{snr_str}"
        
        # 创建目录结构
        test_dir = os.path.join(snr_dir, "Test")
        test_gt_dir = os.path.join(snr_dir, "Test_GT")
        create_directory(test_dir)
        create_directory(test_gt_dir)
        
        # 为当前SNR级别生成100个序列
        for seq_num in tqdm(range(sequences_per_snr), desc=f"Sequences for SNR={snr}", leave=False):
            # 序列编号 (000, 001, ...)
            sequence_name = f"{seq_num:03d}"
            
            # 创建序列文件夹
            seq_folder = os.path.join(test_dir, sequence_name)
            seq_folder_gt = os.path.join(test_gt_dir, sequence_name)
            create_directory(seq_folder)
            create_directory(seq_folder_gt)
            
            # 选择弥撒半径 - 均匀分布在所有序列中
            sigma_idx = (seq_num // (sequences_per_snr // len(sigma_levels))) % len(sigma_levels)
            ...
            debris_velocities = []
            for i in range(num_debris):
                # 使用(seq_num + i)确保每个碎片的速度分布均匀
                v_index = (seq_num + i) % len(velocity_levels)
                velocity = velocity_levels[v_index]
            ...
            # 随机选择基础噪声水平
            base_noise_level = random.choice([0.1, 0.12, 0.15, 0.18])
